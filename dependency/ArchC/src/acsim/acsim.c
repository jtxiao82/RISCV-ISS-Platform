...
void CreateProcessorImpl() {
    ...
    if( ACABIFlag ) {
        fprintf( output, "#include  \"%s_syscall.H\"\n\n", project_name);
        if(ACFaultInjection) // 2018-07-01 Monitor insertion by jtxiao
          fprintf( output, "#include  \"monitor.h\"\n\n");
    }
...
void CreateFaultInjectionInfo() {
    ...
    for (pformat = format_ins_list; pformat != NULL ; pformat = pformat->next) {
      for (pfield = pformat->fields; pfield != NULL; pfield = pfield->next) {
        ...
        fprintf(output, "%s%s_%s_sz %d\n", INDENT[1], pformat->name, pfield->name, pfield->size);
        ++num_pfield;
      }
      fprintf(output, "#define ");
...
void CreateMakefile(){
  ...
  if(ACFaultInjection) {
    ...
    fprintf( output, "ifdef TRACE_FI\n");
    fprintf( output, "OTHER  += -DTRACE_FI\n");
    fprintf( output, "endif\n");
    ...
    fprintf( output, "endif\n");
    fprintf( output, "ifdef CACHEADDR_FI\n");
    fprintf( output, "OTHER  += -DCACHEADDR_FI\n");
    fprintf( output, "endif\n");*/
  }
...
//      fprintf( output, "\t$(MAKE) lib\n\n");
  ...
  if(ACFaultInjection) {
    /*fprintf( output, "#########\n");
    fprintf( output, "# CHIMP #\n");
    fprintf( output, "#########\n\n");
    fprintf( output, ".cpp.o: \n");
    fprintf( output, "\t$(ACXX) $(CFLAGS) $(INC_DIR) -x c++ -g -c $< -o $@\n\n");
  
    fprintf( output, "##########################################\n");
    fprintf( output, "# 2018/04/13                             #\n");
    fprintf( output, "# Due to Aspect++ doesn't support c++11  #\n");
    fprintf( output, "# but unrestricted unions only available #\n");
    fprintf( output, "#  with -std=c++11 or -std=gnu++11       #\n");
    fprintf( output, "##########################################\n\n");
    fprintf( output, "main.o: main.cpp\n");
    fprintf( output, "\t$(CC) $(CFLAGS) -std=c++11 $(INC_DIR) -x c++ -g -c $< -o $@\n\n");
    fprintf( output, "riscv.o: riscv.cpp\n");
    fprintf( output, "\t$(CC) $(CFLAGS) -std=c++11 $(INC_DIR) -x c++ -g -c $< -o $@\n\n");*/
  } else {
    fprintf( output, ".cpp.o:\n");
...
void EmitFetchInit( FILE *output, int base_indent){
  ...
  if (ACPCAddress) {
    ...
    //fprintf( output, "%scerr << \"ArchC: Address out of bounds (pc=0x\" << hex << ac_pc << \").\" << endl;\n",
            //INDENT[base_indent+1]);
    if(ACFaultInjection) {
      fprintf(output, "%s// Monitor the undefinition instruction\n", INDENT[base_indent + 1]);
      fprintf(output, "%smonitor monitorCrash;\n", INDENT[base_indent + 1]);
      fprintf(output, "%smonitorCrash.checkCrash();\n", INDENT[base_indent + 1]);
    }
...
void EmitDecCacheAt(FILE *output, int base_indent) {
  ...
  if( !ACFullDecode ) {
    ...
    // 2018/07/01 - Monitor insertion by jtxiao
    if(ACFaultInjection) {
      fprintf(output, "%s// Monitor the undefinition instruction\n", INDENT[base_indent + 2]);
      fprintf(output, "%smonitor monitorCrash;\n", INDENT[base_indent + 2]);
      fprintf(output, "%smonitorCrash.checkCrash();\n", INDENT[base_indent + 2]);
